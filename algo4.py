...
def get_final_duty():
    while queue:
        ...
        # 조건을 만족하는 duty 찾음
        if now_days_left == 0:
            heapq.heappush(final_duties, (round(now_priority, 2), now_duty[2:]))
            continue
            
        # 조건을 만족하는 duty 충분히 찾음
        if len(final_duties) >= 100:
            return
        ...
        for next_shift in ['T', 'E', 'N', 'O']:
            ...
            if (6 - nurse) + day_nurse_count < 3 and next_shift == 'O':
                continue
...
for nurse in range(6):
    ...
    heapq.heappush(queue, (0, days_left, prev_month_duties[nurse]))
    get_final_duty()  # 한 간호사의 duty 찾기

    team_duties.append(final_duties[0])  # 가장 우선순위 높은 duty를 모든 간호사 duty를 저장할 리스트에 추가
 
    # selected_duty = int(input("선택할 duty 번호를 입력하세요: "))
    # 한 팀의 일별 간호사 배치 현황 기록 (비트 연산으로 기록됨)
    for idx in range(len(final_duties[0][1])):
        if final_duties[0][1][idx] == 'T':
            team_shifts[idx] += 4
        elif final_duties[0][1][idx] == 'E':
            team_shifts[idx] += 2
        elif final_duties[0][1][idx] == 'N':
            team_shifts[idx] += 1
